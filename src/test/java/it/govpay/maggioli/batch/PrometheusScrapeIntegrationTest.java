...
@TestPropertySource(properties = {
        ...
        // override lo @Scheduled reale lancia maggioliJppaNotificationJob quasi
        // subito, in corsa con la chiamata esplicita del test sullo stesso job.
...
class PrometheusScrapeIntegrationTest {
    ...
    @MockitoBean
    private JppaNotificheRepository jppaNotificheRepository = mock(JppaNotificheRepository.class);
    ...
    void scrapeReturnsPrometheusFormatWithApplicationTag() throws Exception {
        ...
        assertThat(response.body()).contains("application=\"govpay-maggioli-jppa-batch\"");
    }

    @Test
    void batchJobExecutionProducesSpringBatchMetrics() throws Exception {
        // Job "a vuoto": nessun dominio abilitato, nessuna partizione da eseguire.
        ...
        when(jppaConfigRepository.findAllByAbilitato(Boolean.TRUE)).thenReturn(List.<JppaConfig>of());
        when(jppaNotificheRepository.findDistinctCodDominio()).thenReturn(List.<String>of());
        ...
        assertThat(scrape).contains("spring_batch_job_name=\"maggioliJppaNotificationJob\"");
    }
