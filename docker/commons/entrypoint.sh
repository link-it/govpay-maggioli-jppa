...
##############################################################################
# GovPay Maggioli JPPA (Notifiche Pagamenti) - Script di Entrypoint
#
...
log_info "========================================"
log_info "Avvio GovPay Maggioli JPPA Batch Processor"
log_info "========================================"
...
GOVPAY_MAGGIOLI_JPPA_MIN_POOL=${GOVPAY_MAGGIOLI_JPPA_MIN_POOL:-2}
GOVPAY_MAGGIOLI_JPPA_MAX_POOL=${GOVPAY_MAGGIOLI_JPPA_MAX_POOL:-10}
    ...
    export SPRING_DATASOURCE_HIKARI_MINIMUM_IDLE="${GOVPAY_MAGGIOLI_JPPA_MIN_POOL}"
    export SPRING_DATASOURCE_HIKARI_MAXIMUM_POOL_SIZE="${GOVPAY_MAGGIOLI_JPPA_MAX_POOL}"

    # Export per init_maggioli_jppa_db.sh
...
if [ "${GOVPAY_MAGGIOLI_JPPA_POP_DB_SKIP:-TRUE}" != "TRUE" ] && [ -n "${GOVPAY_DB_TYPE}" ]; then
    log_info "Esecuzione script di inizializzazione database..."
    /usr/local/bin/init_maggioli_jppa_db.sh
    if [ $? -ne 0 ]; then
...
# Verifica se modalita' CRON e' abilitata tramite GOVPAY_MAGGIOLI_JPPA_BATCH_USA_CRON
...
case "${GOVPAY_MAGGIOLI_JPPA_BATCH_USA_CRON,,}" in
    si|yes|1|true)
    ...
    *)
        ...
        INTERVALLO_MINUTI=${GOVPAY_MAGGIOLI_JPPA_BATCH_INTERVALLO_CRON:-120}
        SCHEDULER_MAGGIOLIJPPANOTIFICATIONJOB_FIXEDDELAYSTRING=$((INTERVALLO_MINUTI * 60 * 1000))

        export SERVER_PORT SCHEDULER_MAGGIOLIJPPANOTIFICATIONJOB_FIXEDDELAYSTRING
        log_info "Porta Actuator: ${SERVER_PORT}, Intervallo scheduler: ${INTERVALLO_MINUTI} minuti (${SCHEDULER_MAGGIOLIJPPANOTIFICATIONJOB_FIXEDDELAYSTRING}ms)"
        ;;
esac
...
JVM_MEMORY_OPTS="-XX:MaxRAMPercentage=${GOVPAY_MAGGIOLI_JPPA_JVM_MAX_RAM_PERCENTAGE:-${DEFAULT_MAX_RAM_PERCENTAGE}}"
[ -n "${GOVPAY_MAGGIOLI_JPPA_JVM_INITIAL_RAM_PERCENTAGE}" ] && JVM_MEMORY_OPTS="$JVM_MEMORY_OPTS -XX:InitialRAMPercentage=${GOVPAY_MAGGIOLI_JPPA_JVM_INITIAL_RAM_PERCENTAGE}"
[ -n "${GOVPAY_MAGGIOLI_JPPA_JVM_MIN_RAM_PERCENTAGE}" ] && JVM_MEMORY_OPTS="$JVM_MEMORY_OPTS -XX:MinRAMPercentage=${GOVPAY_MAGGIOLI_JPPA_JVM_MIN_RAM_PERCENTAGE}"
[ -n "${GOVPAY_MAGGIOLI_JPPA_JVM_MAX_METASPACE_SIZE}" ] && JVM_MEMORY_OPTS="$JVM_MEMORY_OPTS -XX:MaxMetaspaceSize=${GOVPAY_MAGGIOLI_JPPA_JVM_MAX_METASPACE_SIZE}"
[ -n "${GOVPAY_MAGGIOLI_JPPA_JVM_MAX_DIRECT_MEMORY_SIZE}" ] && JVM_MEMORY_OPTS="$JVM_MEMORY_OPTS -XX:MaxDirectMemorySize=${GOVPAY_MAGGIOLI_JPPA_JVM_MAX_DIRECT_MEMORY_SIZE}"
...
log_info "Java: MaxRAMPercentage=${GOVPAY_MAGGIOLI_JPPA_JVM_MAX_RAM_PERCENTAGE:-${DEFAULT_MAX_RAM_PERCENTAGE}}%"
log_info "========================================"
...
JAR_FILE=$(find /opt/govpay-maggioli-jppa -name "*.jar" -type f | head -n 1)

if [ -z "${JAR_FILE}" ]; then
    log_error "Nessun file JAR trovato in /opt/govpay-maggioli-jppa"
    exit 1
fi

log_info "Avvio: ${JAR_FILE}"
log_info "Driver JDBC: ${GOVPAY_DS_JDBC_LIBS}"
log_info "========================================"
