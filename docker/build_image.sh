...
Sorgente:
-v <VERSIONE>  : Imposta la versione della release binaria da utilizzare per il build (default: ${LATEST_GOVPAY_MAGGIOLI_JPPA_RELEASE})
...
LATEST_LINK="$(curl -qw '%{redirect_url}\n' https://github.com/link-it/govpay-maggioli-jppa/releases/latest 2> /dev/null)"
LATEST_GOVPAY_MAGGIOLI_JPPA_RELEASE="${LATEST_LINK##*/}"
...
[ -n "${CUSTOM_GOVPAY_HOME}" ] && DOCKERBUILD_OPTS=(${DOCKERBUILD_OPTS[@]} '--build-arg' "govpay_maggioli_jppa_home=${CUSTOM_GOVPAY_HOME}")
[ -n "${CUSTOM_GOVPAY_LOG}" ] && DOCKERBUILD_OPTS=(${DOCKERBUILD_OPTS[@]} '--build-arg' "govpay_maggioli_jppa_log=${CUSTOM_GOVPAY_LOG}")
if [ -n "${CUSTOM_RUNTIME}" ]
then
